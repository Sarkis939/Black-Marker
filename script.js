...
function createEditableObject(obj) {
  ...
  div.style.resize = obj.type === "sticky" ? "both" : "none";
...
function start(e){
  ...
  if(tool === "text"){
    const obj = { type:"text", x, y, width:200, height:40, text:"", color };
    objects.push(obj);
    createEditableObject(obj);
  }

  if(tool === "sticky"){
    const obj = { type:"sticky", x, y, width:180, height:120, text:"", color };
    objects.push(obj);
    createEditableObject(obj);
  }
  ...
  objects.forEach(obj => {
    ...
    obj.points.forEach(p => {
      ...
      if(dist > eraserRadius){
        ...
        if(current.length > 1){
          segments.push({ type:"path", points:[...current], color:obj.color });
        }
        current = [];
      }
    });
...
function zoomCanvas(e){
  ...
  objects.forEach(obj => {
    if(obj._div){
      obj._div.style.left = obj.x * camera.zoom + camera.x + "px";
      obj._div.style.top = obj.y * camera.zoom + camera.y + "px";
      obj._div.style.width = obj.width * camera.zoom + "px";
      obj._div.style.height = obj.height * camera.zoom + "px";
    }
  });
...
function draw(){
  ...
  ctx.clearRect(-camera.x/camera.zoom,-camera.y/camera.zoom,
                canvas.width/camera.zoom,canvas.height/camera.zoom);
